...
public class RobotContainer {
    ...
    public RobotContainer() {
        ...
        NamedCommands.registerCommand("Acquire",
                runAquire()
                        .andThen(coral.runMotorCommand(.7).alongWith(Commands.waitUntil(coral.hasGamePieceSupplier()))
        ...
        NamedCommands.registerCommand("Score Outfeed",
                coral.runMotorCommand(-.8).alongWith(Commands.waitSeconds(1)).andThen(coral.runMotorCommand(0)));
    ...
    private void configureBindings() {
        ...
        drivetrain.setDefaultCommand(
                ...
                drivetrain.applyRequest(
                        () -> drive.withVelocityX(
                                scaleDriverController(-driverController.getLeftY(), yLimiter, 0.25)
                                        * MaxSpeed) // Drive
                                // forward
                                // with
                                // negative Y (forward)
                                .withVelocityY(scaleDriverController(-driverController.getLeftX(), xLimiter, 0.25)
                                        * MaxSpeed) // Drive
                                                    // left
                                                    // with
                                                    // negative
                                                    ...
                                                    // (left)
                                .withRotationalRate(
                                        scaleDriverController(-driverController.getRightX(), thetaLimiter, 0.25)
    ...
    public Command runAquire() {
        return elevator.runToPosition(17).alongWith(arm.runToPositionCommand(Units.degreesToRadians(235)))
                .alongWith(Commands.waitUntil(armAndElevatorKosher()));
    }
    ...
    public Command realDrivetrainStop() {
        return drivetrain
                .runOnce(() -> drivetrain.setControl(drive.withVelocityX(0).withVelocityY(0).withRotationalRate(0)));
    }
}
