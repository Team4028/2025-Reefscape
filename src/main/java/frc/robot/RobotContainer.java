...
public class RobotContainer {
    ...
    private void configureBindings() {
        ...
        driverController.leftTrigger().onTrue(coral.runMotorCommand(.7)
                .alongWith(Commands.waitUntil(
                        coral.hasGamePieceSupplier()))
                .andThen(coral.runMotorCommand(0))).onFalse(coral.runMotorCommand(0));
        ...
        // driverController.a()
        // .onTrue(AutoSequencing.autoScoreReef(drive, armistice, coral,
        // () -> new Pose2d(13.72, 5.1, Rotation2d.fromDegrees(330)),
        // () -> ArmisticePositions.L4));
        driverController.a().onTrue(Commands.defer(this::runToClosestReef,
                Set.<Subsystem>of(drive, armistice.getArm(), armistice.getElevator(), coral)));

        driverController.rightStick().onTrue(drive.runOnce(() -> {
        }));
    ...
    public Command runToClosestReef() {
        return AutoSequencing.autoScoreReef(drive, armistice, coral, () -> drive.closestReefPose(true),
                () -> ArmisticePositions.L4);
    }
