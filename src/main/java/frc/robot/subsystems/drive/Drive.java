...
public class Drive extends SubsystemBase {
    ...
    private static final double ROBOT_MASS_KG = 40.5344;
    ...
    public Command llLineup2d(Limelight sourceLimelight, int tagID, DoubleSupplier targetTx, DoubleSupplier targetTy,
        ...
        return runOnce(() -> {
            ...
            double rotErr = targetRot.getAsDouble() - tagRot;

            if (xErr <= .5 && yErr <= .5 && rotErr < 1) {
                isFinished2dAlign = true;
            } else
                isFinished2dAlign = false;
            if (tv) {
                ...
                rotOutput = Math.abs(rotErr) > 1 ? rotPid.calculate(tagRot, targetRot.getAsDouble()) : 0;
                var xTmp = xOutput;
