...
public class AutoSequencing {
    public static final Command autoScoreReef(Drive drive, Armistice armistice, CoralManipulator coral,
            Supplier<ReefPositions> reefPosition, Supplier<ArmisticePositions> scorePosition) {
        return drive.pathfindToPose(reefPosition.get().pose).andThen(armistice.runToPositionCommand(scorePosition.get())
                ...
                .alongWith(drive.translateToPositionWithPID(reefPosition.get().pose)));
    }
}
